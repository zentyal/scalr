Scalr.regPage('Scalr.ui.monitoring.view', function (loadParams, moduleParams) {
	...
	function addToolMenu (farm, watchername, type, role) {
		if(/*Scalr.flags.newDashboardTest == true*/moduleParams['dashboard_enabled'] == '1') {
			if (panel.down('#' + watchername + farm + role + 'tool'))
				panel.down('#' + watchername + farm + role + 'tool').destroy();
			panel.down('#' + watchername + farm + role).addTool({
				type: 'dashboard',
				tooltip: 'Add to dashboard',
				itemId: watchername + farm + role + 'tool',
				handler: function (e, toolEl, p, t) {
					var data = Ext.encode({params: {'farmid': farm, 'watchername': watchername, 'graph_type': type, 'role': role, 'width': p.width, 'height': p.height, 'title': p.title + ' ( ' + type + ' )'}, name: 'dashboard.monitoring', url: '' });
					Scalr.Request({
						processBox: {
							type: 'action',
							msg: 'Adding new widget to dashboard. Please wait...'
						},
						url: '/dashboard/xUpdatePanel',
						params: {'widget': data},
						success: function (data, response, options) {
							Scalr.message.keepMessagesFlag = true;
	...
	function addDockMenu(farm, watchername, type, role) {
		if(panel.down('#' + watchername + farm + role)){
			if (panel.down('#' + watchername + farm + role).dockedItems.getAt(1))
			...
			Scalr.Request({
				...
				success: function (data, response, options) {
					if(panel.down('#' + watchername + farm + role)){
						...
						addToolMenu(farm, watchername, type, role);
