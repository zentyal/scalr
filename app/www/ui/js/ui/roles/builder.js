Scalr.regPage('Scalr.ui.roles.builder', function (loadParams, moduleParams) {
	...
	var checkboxBehaviorListener = function() {
		if (this.behavior == 'mysql') {
			if (this.pressed) {
				if (moduleParams['images'][result['platform']][result['imageId']]['name'] == 'Ubuntu 12.04') {
					form.down('[name="mysqlServerType"]').store.loadData([['mysql', 'MySQL 5.5']]);
				} else {
					form.down('[name="mysqlServerType"]').store.load({ data: [['mysql', 'MySQL 5.x'], ['percona', 'Percona Server 5.1']] });
				}
				form.down('[name="mysqlServerType"]').reset();
