Ext.define('Scalr.ui.ScriptEventView', {
	...
	scalrModuleData: {},
	store: {
		fields: [ 'script_id', 'script_name', 'event_name', 'target', 'issync', 'timeout', 'version', 'params', 'order_index' ],
	...
	columns: [
		{ width: 30, sortable: false, xtype: 'templatecolumn', tpl:
			'<img src="' +
				'<tpl if="target != &quot;&quot;">/ui/images/ui/farms/scripting/execution.png</tpl>' +
				'<tpl if="target == &quot;&quot;">/ui/images/ui/farms/scripting/execution_disable.png</tpl>' +
			'">'
		},
...
Ext.define('Scalr.ui.ScriptField', {
	...
	scalrLoadRecord: function (record) {
		...
		this.down('#when').setValue(this.scalrModuleData['events'][record.get('event_name')]);
		this.down('#do').setValue(this.scalrModuleData['scripts'][record.get('script_id')]['description']);
		...
		var revisions = Scalr.utils.CloneObject(this.scalrModuleData['scripts'][record.get('script_id')]['revisions']);
			...
			handler: function () {
				if (this.up('scriptfield').getForm().isValid()) {
					var values = this.up('scriptfield').getForm().getValues(),
						data = this.up('scriptfield').scalrModuleData.scripts[values.script_id],
