Scalr.regPage('Scalr.ui.servers.view', function (loadParams, moduleParams) {
	var store = Ext.create('store.store', {
		fields: [
			...
			'initDetailsSupported', 'isInitFailed', 'la_server', 'launch_error'
		],
		proxy: {
			type: 'scalr.paging',
	...
	return Ext.create('Ext.grid.Panel', {
		...
		columns: [
			...
			{ header: "Cloud Server ID", width: 100, dataIndex: 'cloud_server_id', sortable: false, hidden: true, xtype:'templatecolumn',  tpl:
				...
				'<tpl if="!cloud_server_id"><img src="/ui/images/icons/false.png" /></tpl>'
			},
			...
			{ header: "Status", width: 150, dataIndex: 'status', sortable: true, xtype: 'templatecolumn', tpl:
				'<tpl if="initDetailsSupported">' +
					...
					'<tpl if="!isInitFailed">' +
						'<tpl if="status == &quot;Pending&quot; || status == &quot;Initializing&quot;"><img src="/ui/images/icons/running.gif" style="vertical-align:middle; margin-top:-2px;" /> <a href="#/operations/details?serverId={server_id}&operation=Initialization">{status}</a></tpl>' +
			...
			{ header: "Agent", width: 38, dataIndex: 'agent_version', sortable: false, xtype: 'templatecolumn',  align: 'center', tpl:
				...
				request: {
					...
					dataHandler: function (records) {
						...
						for (var i = 0, len = records.length; i < len; i++) {
							...
							this.confirmBox.objects.push(records[i].get('server_id'));
						}
