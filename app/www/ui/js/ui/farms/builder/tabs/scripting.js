	...
	return Ext.create('Scalr.ui.FarmsBuilderTab', {
		...
		items: [{
			...
			items: {
				...
				flex: 1,
				store: {
					fields: [ 'script_id', 'script', 'event', 'target', 'issync', 'timeout', 'version', 'params', 'order_index', 'system' ],
					proxy: 'object'
				},
				border: true,
					...
					selectionchange: function (dataview, selections) {
						...
						if (selections.length) {
							...
							for (var i in params) {
								...
								if (f)
									f.setValue(params[i]).setReadOnly(readOnly);
							}
