...
Ext.define('Scalr.ui.GridOptionsColumn', {
	...
	showOptionsMenu: function (view, record) {
		...
		var btnEl = Ext.get(view.getNode(record)).down('div.scalr-ui-grid-options-column-btn'), xy = btnEl.getXY(), sizeX = xy[1] + btnEl.getHeight() + this.optionsMenu.getHeight();
		// menu shouldn't overflow window size
		if (sizeX > Scalr.application.getHeight()) {
			xy[1] -= sizeX - Scalr.application.getHeight();
		}
		this.optionsMenu.setPosition([xy[0] - (this.optionsMenu.getWidth() - btnEl.getWidth()), xy[1] + btnEl.getHeight()]);
	},
...
Ext.define('Scalr.ui.GridSelectionModel', {
	...
	getHeaderConfig: function() {
		...
		c.renderTpl =
			'<div id="{id}-titleEl" class="' + Ext.baseCSSPrefix + 'column-header-inner scalr-ui-grid-selected-column-btn">' +
	...
	onHeaderClick: function(headerCt, header, e) {
		if (e.getTarget('div.scalr-ui-grid-selected-column-btn-trigger')) {
			...
			this.selectedMenu.setPosition([xy[0] - (this.selectedMenu.getWidth()  - btnEl.getWidth()), xy[1] + btnEl.getHeight()]);
			e.stopEvent();
		} else {
			this.callParent(arguments);
		}
