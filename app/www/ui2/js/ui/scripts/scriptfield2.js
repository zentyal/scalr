Ext.define('Scalr.ui.RoleScriptingPanel', {
	...
	items: [{
		xtype: 'grid',
		style: 'background-color:#A8B6C6;border-radius:4px',
		hideHeaders: true,
		cls: 'x-panel-columned-leftcol x-grid-shadow x-grid-role-scripting',
		padding: 9,
		width: 500,
		multiSelect: true,
		selModel: {
		...
		features: [{
			...
			getAdditionalData: function(data, rowIndex, record, orig) {
				...
				switch (target) {
					...
					case 'roles':
						var roleIds = target == 'role' ? [this.grid.up('scriptfield2').farmRoleId] : (record.get('target_roles') || []),
							roles = [],
							rolesStore = this.grid.next().getForm().findField('target_roles').getStore();
					...
					case 'behaviors':
						var bahaviorIds = record.get('target_behaviors') || [],
							behaviors = [],
							behaviorsStore = this.grid.next().getForm().findField('target_behaviors').getStore();
				...
				return {
					rowBody: '<div style="padding:0 51px 5px">'+name+'</div>',
			...
			viewready: function() {
				...
				me.getSelectionModel().on('focuschange', function(gridSelModel){
					if (!me.disableOnFocusChange) {
						var formPanel = me.next();
						if (gridSelModel.lastFocused) {
							if (gridSelModel.lastFocused != formPanel.getRecord()) {
								formPanel.loadRecord(gridSelModel.lastFocused);
							}
						} else {
							formPanel.deselectRecord();
						}
					}
				});
			},
		...
		viewConfig: {
			plugins: {
				...
				emptyTextNoItems: 'Click <a class="add-link" href="#">here</a> to create your first orchestration rule.',
		...
		dockedItems: [{
			...
			items: [{
				...
				handler: function() {
					...
					form.loadRecord(grid.getStore().createModel({issync: '1', order_index: 10}));
				}
			}]
		}]
	}, {
		...
		items: [{
			...
			items: [{
				...
				items: [{
					xtype: 'component',
					style: 'color: #212B3D;font:17px/22px arial,helvetica,tahoma,sans-serif;text-shadow: 0 1px #FFFFFF',
					html: 'Trigger'
				}, {
					...
					listeners: {
						change: function(comp, value) {
							...
							if (scriptRecord) {
								...
								form.findField('issync').setValue(scriptRecord.get('issync'));
				...
				items: [{
					...
					items: [{
						...
						queryMode: 'local',
						store: {
							fields: ['farm_role_id', 'platform', 'cloud_location', 'role_id',  'name', {name: 'title', convert: function(v, rec){return rec.data.name + ' (' + rec.data.cloud_location +')'}}],
							proxy: 'object'
						},
						flex: 1,
						listeners: {
		...
		updateRecord: function(fields) {
			...
			if (Ext.Object.getSize(values) && record) {
				var grid = this.prev(),
					scrollTop = grid.getView().el.getScroll().top;
				grid.disableOnFocusChange = true;
				grid.suspendLayouts();
				grid.getView().getFeature('grouping').disable();
				record.set(values);
				if (record.store === undefined) {
					grid.getStore().add(record);
					grid.getSelectionModel().setLastFocused(record);
				}
				grid.getView().getFeature('grouping').enable();
				grid.disableOnFocusChange = false;
				grid.resumeLayouts(true);
				grid.getView().el.scrollTo('top', scrollTop);
