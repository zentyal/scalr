Scalr.regPage('Scalr.ui.roles.builder2', function (loadParams, moduleParams) {
	...
	var beautifyOsFamily = function(osfamily) {
		var map = {
			ubuntu: 'Ubuntu',
			centos: 'CentOS',
			gcel: 'GCEL',
	...
	var panel = Ext.create('Ext.panel.Panel', {
		...
		items:[{
			...
			items: [{
				...
				items: [{
					xtype: 'label',
					text: '1. Location and Operating System',
					...
					listeners: {
						change: function(comp, value) {
							...
							panel.down('#locationmap').selectLocation(result.platform, result.cloud_location, locations);
						}
					}
				},{
			...
			margin: '12 12 12 0',
			padding: '16 0 16 32',
			bodyPadding: '0 32 0 0',
			items: [{
				xtype: 'label',
				text: '2. General Info and Software',
				cls: 'x-panel-header-text',
				margin: '50 0'
			},{
				xtype: 'textfield',
				...
				defaults: {
					...
					renderTpl:
						'<div class="x-btn-custom" style="height:100%" id="{id}-btnEl">'+
							'<div class="x-btn-icon scalr-ui-icon-behavior-large scalr-ui-icon-behavior-large-{name}"></div><div class="x-btn-title" style="text-align:center;margin:2px 0 0">{title}</div>'+
						'</div>',
					margin: '0 10 10 0',
					tooltip: '',
					listeners: {
		...
		onSelectPlatform: function(platformId) {
			...
			toggleBehaviors(images.length ? false : true);
			panel.down('#save')[images.length ? 'disable' : 'enable']();
			if (result) {
			...
			if (images.length) {
				for (var i=0, len=images.length; i<len; i++) {
					var image = images[i],
						imageOS = beautifyOsFamily(image.os_family) + ' ' + image.os_version; 
					if (!added[imageOS]) {
						compImages.add({
							xtype: 'custombutton',
							width: 95,
							...
							renderTpl:
								'<div class="x-btn-custom" style="height:100%" id="{id}-btnEl">'+
									'<div class="x-btn-icon scalr-ui-icon-osfamily-large scalr-ui-icon-osfamily-large-{osFamily}"></div><div class="x-btn-title" style="text-align:center;margin:2px 0 0">{title}</div>'+
								'</div>',
							renderData: {
		...
		refreshBehaviors: function() {
			...
			for (var i=0, len=behaviors.length; i<len; i++) {
				...
				if (enabled) {
					panel.down('[behavior="'+item.name+'"]').enable();
				}  else {
					var btn = panel.down('[behavior="'+item.name+'"]');
					btn.toggle(false).disable();
					var message = '';
					if (disableInfo.reason == 'os') {
						message = '<b>' + beautifyBehavior(item.name) + '</b> cannot be used together with <b style="white-space:nowrap">' + beautifyOsFamily(result.osfamily) + ' ' + result.osversion + '</b>.';
		...
		listeners: {
			...
			selectname: function(osFamily, osVersion, select) {
				...
				toggleBehaviors(select);
				panel.down('#save').setDisabled(!select);
				if (select) {
					panel.refreshBehaviors();
				}
			}
