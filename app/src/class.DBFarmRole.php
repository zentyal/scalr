<?php
	...
	class DBFarmRole
	{
        ...
        public function SetServiceConfiguration($behavior, $config) {
            ...
            $this->SetSetting(self::SETTING_SYSTEM_NEW_PRESETS_USED, 1);
		...
		public function SetScripts(array $scripts, array $params = array())
		{
			...
			if (count($params) > 0) {
				foreach ($params as $param) {
					...
					if ($hash) {
						...
						$roleParams = $this->DB->GetOne("SELECT params FROM role_scripts WHERE role_id = ? AND `hash` = ?", array($roleId, $hash));
			...
			if (count($scripts) > 0) {
				foreach ($scripts as $script) {
					...
					if ($event_name && $script['script_id'])
					{
						...
						if ($script['target'] == SCRIPTING_TARGET::ROLES || $script['target'] == SCRIPTING_TARGET::BEHAVIORS) {
							
							$targetType = ($script['target'] == SCRIPTING_TARGET::ROLES) ? 'farmrole' : 'behavior';
							$varName = ($script['target'] == SCRIPTING_TARGET::ROLES) ? 'target_roles' : 'target_behaviors';
