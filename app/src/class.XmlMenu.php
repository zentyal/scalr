...
class XmlMenu
{
	...
	private function GetExtJSMenuItemsRecursive ($Context, &$ret)
	{
		...
		{
			$xj_item = null;
			if ("separator" == $Item->getName() && !(int)$Item->attributes()->hidden)
				$xj_item = "-";
			else if ("item" == $Item->getName() && !(int)$Item->attributes()->hidden)
			{
				$xj_item = array
				(
					"text" => (string)$Item, 
				...
				if ((string)$Item->attributes()->iconCls)
					$xj_item['iconCls'] = (string)$Item->attributes()->iconCls;
			}
			else if ("node" == $Item->getName() && !(int)$Item->attributes()->hidden)
			{
				$xj_item = array
				(
