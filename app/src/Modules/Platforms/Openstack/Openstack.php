<?php
	class Modules_Platforms_Openstack implements IPlatformModule
	{
		...
		public function GetServerIPAddresses(DBServer $DBServer)
		{
			...
			if (!$remoteIp) {
				foreach ($result->server->addresses->internet as $addr)
					if ($addr->version == 4) {
						$remoteIp = $addr->addr;
						break;
					}
		...
		public function GetServerExtendedInformation(DBServer $DBServer)
		{
			try
			{
				try	{
					$osClient = $this->getOsClient($DBServer->GetEnvironmentObject(), $DBServer->GetProperty(OPENSTACK_SERVER_PROPERTIES::CLOUD_LOCATION));
					$iinfo = $osClient->serverGetDetails($DBServer->GetProperty(OPENSTACK_SERVER_PROPERTIES::SERVER_ID));
		        ...
		        if ($iinfo)
		        {
		        	return array(
			        	...
			        	'Image ID'				=> $iinfo->server->image->id,
			        	'Flavor ID'				=> $iinfo->server->flavor->id,
