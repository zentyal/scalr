	...
	class Modules_Platforms_Openstack extends Modules_Platform implements IPlatformModule
	{
        ...
        private function GetServerSecurityGroupsList(DBServer $DBServer, $osClient)
        {
            ...
            */
             ...
             */
            /*
            if ($sgroups[$scalrSecuritySettings['ec2']['security_group_name']]) {
            
                $osClient->servers->securityGroups->delete($sgroups[$scalrSecuritySettings['ec2']['security_group_name']]->id);
                
                unset($sgroups[$scalrSecuritySettings['ec2']['security_group_name']]);    
            }
            */
            ...
            if (!$sgroups[$scalrSecuritySettings['ec2']['security_group_name']]) {
                try {
                    $group = $osClient->servers->securityGroups->create($scalrSecuritySettings['ec2']['security_group_name'], _("Security group for Roles Builder"));
                    $groupId = $group->id;
                }
                catch(Exception $e) {
