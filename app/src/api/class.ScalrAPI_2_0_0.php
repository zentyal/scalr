<?php
	...
	class ScalrAPI_2_0_0 extends ScalrAPICore
	{
		...
		public function FarmGetStats($FarmID, $Date = null)
		{
			...
			$rows = $this->DB->Execute("SELECT SUM(`usage`) AS `usage`, instance_type, cloud_location, month, year FROM servers_stats WHERE farm_id=? {$filter_sql} GROUP BY instance_type, month, year, cloud_location", 
				array($FarmID)
			);
			while ($row = $rows->FetchRow())
			{
