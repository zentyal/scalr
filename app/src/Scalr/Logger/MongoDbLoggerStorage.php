...
class MongoDbLoggerStorage implements LoggerStorageInterface
{
	...
	private function getLogRecord ($data)
	{
		...
		if (isset($data['datatype'])) {
			$dclass = __NAMESPACE__ . '\\AuditLog\\Documents\\' . $data['datatype'] . 'Document';
			$v = $dclass::createFromArray($data['data']);
			$record->setData($v);
		}
	...
	public function write(LogRecord $record)
	{
		...
		$r = array(
			...
			'data'      => $data instanceof AbstractAuditLogDocument ? $data->toArray() : array(),
