...
class ElbTest extends ElbTestCase
{
	...
	/**
	 * Set instanceId if you wish to test instance related tests
	 */
	const F_INSTANCE_ID = 'i-39b61b42';
	...
	public function setUp()
	{
		...
		$this->assertNotEmpty($this->elb->getAws()->getAccessKeyId());
		$this->assertNotEmpty($this->elb->getAws()->getSecretAccessKey());
	}
	...
	/**
	 * @test
	 * @expectedException   Scalr\Service\Aws\ElbException
	 */
	...
	public function testCreateLoadBalancers()
	{
		if ($this->isSkipFunctionalTests()) {
			$this->markTestSkipped('testCreateLoadBalancers is skipped.');
		}
		$loadBalancer1 = $this->createLoadBalancerGently('phpunit-test-load-balancer-1', array(
			array(
				'loadBalancerPort' => 80,
				'instancePort' => 1024,
				'protocol' => 'HTTP',
			...
			Aws::REGION_US_EAST_1 . 'a'
		));
		$loadBalancer2 = $this->createLoadBalancerGently('phpunit-test-load-balancer-2', array(
			array(
	...
	{
		$exeption = null;
		$repo = $this->getEntityManager()->getRepository('Elb:LoadBalancerDescription');
		$lb2 = $repo->find('phpunit-test-load-balancer-2');
		$this->assertInstanceOf(self::CLASS_LOAD_BALANCER_DESCRIPTION_DATA, $lb2);
		try {
			$lbSameName = $this->elb->loadBalancer->create('phpunit-test-load-balancer-2', array(
				array(
	...
	public function testDescribeLoadBalancers()
	{
		...
		$one = $repository->find('phpunit-test-load-balancer-1');
		$this->assertInstanceOf(self::CLASS_LOAD_BALANCER_DESCRIPTION_DATA, $one);
		$this->assertEquals('phpunit-test-load-balancer-1', $one->loadBalancerName, 'Load Balancer name does not match.');
		try {
		...
		$two = $repository->find('phpunit-test-load-balancer-2');
		$this->assertInstanceOf(self::CLASS_LOAD_BALANCER_DESCRIPTION_DATA, $two);
		$this->assertEquals('phpunit-test-load-balancer-2', $two->loadBalancerName, 'Load Balancer name does not match.');
	}
	...
	{
		$repository = $this->getEntityManager()->getRepository('Elb:LoadBalancerDescription');
		$ret = $this->elb->loadBalancer->delete('phpunit-test-load-balancer-1');
		$one = $repository->find('phpunit-test-load-balancer-1');
		$this->assertNull($one);
		$two = $repository->find('phpunit-test-load-balancer-2');
	...
	public function testDetachLoadBalancerFromSubnets()
	{
		...
		$repo = $this->getElbLoadBalancerDescripitonRepository();
		$elb = $this->getElbMock();
	...
	{
		$loadBalancerName = 'phpunit-test-load-balancer-4';
		...
		try {
			...
			foreach ($loadBalancer->listenerDescriptions as $listenerDescription) {
				$a[(string) $listenerDescription->listener] .= $listenerDescription->listener;
			}
			$mapping = array();
			...
			$ret = $this->elb->loadBalancer
				->enableAvailabilityZones($loadBalancerName, Aws::REGION_US_EAST_1 . 'b')
			;
			$this->assertContains(Aws::REGION_US_EAST_1 . 'b', $ret);
			$this->assertEquals($ret, $loadBalancer->availabilityZones);

			$ret = $loadBalancer->disableAvailabilityZones(Aws::REGION_US_EAST_1 . 'a');
			$this->assertNotContains(Aws::REGION_US_EAST_1 . 'a', $ret);
			$this->assertEquals($ret, $loadBalancer->availabilityZones);

			//Tests that depends from Instance which should be previously created.
			if (self::F_INSTANCE_ID) {
				$instanceList = $loadBalancer->registerInstances(self::F_INSTANCE_ID);
				$this->assertContains(self::F_INSTANCE_ID, $instanceList->getQueryArray());
				...
				$this->assertContains(self::F_INSTANCE_ID, $instanceStateList->getQueryArray());
				...
				$dInstanceList = $loadBalancer->deregisterInstances(self::F_INSTANCE_ID);
	...
	public function testAvailabilityZones()
	{
		...
		$loadBalancer->disableAvailabilityZones(Aws::REGION_US_EAST_1 . 'b');
		$this->assertEquals(array(
			Aws::REGION_US_EAST_1 . 'a',
			Aws::REGION_US_EAST_1 . 'c'
		), $loadBalancer->availabilityZones);
	...
	public function testConfigureHealthCheck()
	{
		...
		//Checks if it's ok
		$this->assertEquals($original, $loadBalancer->healthCheck);
		...
		$this->assertEquals($expected, $loadBalancer->healthCheck);
		unset($expected);
		unset($result);
		$loadBalancer->delete();
	}
