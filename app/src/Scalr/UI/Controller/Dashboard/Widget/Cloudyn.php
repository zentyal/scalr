...
class Scalr_UI_Controller_Dashboard_Widget_Cloudyn extends Scalr_UI_Controller_Dashboard_Widget
{
	...
	{
		if (empty(CONFIG::$CLOUDYN_MASTER_EMAIL)) {
	...
	public function xSetupAction()
	{
		...
		$masterEmail = CONFIG::$CLOUDYN_MASTER_EMAIL;
		...
		if (!$isCloudynEnabled) {
			...
			$cy = new Cloudyn($userEmail, $userPassword, isset(CONFIG::$CLOUDYN_ENVIRONMENT) ? CONFIG::$CLOUDYN_ENVIRONMENT : 'PROD');
