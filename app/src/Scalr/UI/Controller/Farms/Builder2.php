...
class Scalr_UI_Controller_Farms_Builder2 extends Scalr_UI_Controller
{
	...
	public function xBuildAction()
	{
		...
		foreach ($this->getParam('roles') as $role) {
            ...
            if ($role['farm_role_id'] && !$role['params']) {
                try {
		...
		foreach ($this->getParam('roles') as $role) {
			...
			//try {
				$dbFarmRole->getStorage()->setConfigs($role['storages']);
	...
	public function getFarm($farmId)
	{
		...
		foreach ($dbFarm->GetFarmRoles() as $dbFarmRole) {
			...
			$farmRoles[] = array(
				...
				'storages'      => $dbFarmRole->getStorage()->getConfigs()
			);
		}
