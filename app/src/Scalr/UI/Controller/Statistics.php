	...
	public function getInstancePrice()
	{
		...
		foreach ((array)$priceList as $row)
		{
			foreach ($row->instanceTypes as $type)
			{
				...
				{
					$size = (array)$size;
					$region = str_replace(array('us-east', 'apac-sin', 'apac-syd', 'apac-tokyo', 'eu-ireland'), array('us-east-1', 'ap-southeast-1', 'ap-southeast-2', 'ap-northeast-1', 'eu-west-1'), $row->region);
					if ($region == "us-west")
						$region = 'us-west-1';
					$iType = $compliance[$type['type']];
					if ($compliance[$size['size']] == '8xlarge')
						$iType = 'cc2';
					$price[$region][$iType.'.'.$compliance[$size['size']]] = $size['valueColumns'][0]->prices->USD;
				}
