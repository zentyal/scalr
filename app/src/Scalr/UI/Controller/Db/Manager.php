...
class Scalr_UI_Controller_Db_Manager extends Scalr_UI_Controller
{
    ...
    private function getDbStorageStatus(DBServer $masterServer, $behavior)
    {
        ...
        $retval['growSupported'] = in_array($retval['engine'], array(MYSQL_STORAGE_ENGINE::EBS, MYSQL_STORAGE_ENGINE::RAID_EBS)) && in_array($behavior, array(ROLE_BEHAVIORS::MYSQL2, ROLE_BEHAVIORS::PERCONA));
    ...
    public function dashboardAction()
    {
        ...
        if (in_array($data['dbType'], array(ROLE_BEHAVIORS::MYSQL, ROLE_BEHAVIORS::MYSQL2, ROLE_BEHAVIORS::PERCONA)))
	    ...
	    if (!$masterServer) {
		    $this->response->failure('Master server not found');
		    return;
	    }
        ...
        if ($dbFarmRole->GetSetting(Scalr_Db_Msr::DATA_BUNDLE_ENABLED)) {
            $data['bundles'] = array(
            	...
            	'inProgress' => array(
            		'status' => $dbFarmRole->GetSetting(Scalr_Db_Msr::DATA_BUNDLE_IS_RUNNING),
            	...
            	'next' => (!$nextTime) ? "Within 30 minutes" : Scalr_Util_DateTime::convertTz((int)$nextTime, 'd M Y \a\\t H:i:s'),
            	'schedule' => "Every {$period} hours"
            );
        ...
        if ($dbFarmRole->GetSetting(Scalr_Db_Msr::DATA_BACKUP_ENABLED)) {
            $data['backups'] = array(
            	'history' => $this->db->GetAll("SELECT *, UNIX_TIMESTAMP(date) as date FROM services_db_backups_history WHERE `farm_role_id` = ? AND `operation` = ? ORDER BY id ASC", array(
            		$dbFarmRole->ID, 'backup'		
            	)),
            	'inProgress' => array(
            		'status' => $dbFarmRole->GetSetting(Scalr_Db_Msr::DATA_BACKUP_IS_RUNNING),
        ...
        */
        ...
        foreach ($dbFarmRole->GetServersByFilter() as $dbServer) {
            ...
            try {
                ...
                if (in_array($data['dbType'], array(ROLE_BEHAVIORS::MYSQL2, ROLE_BEHAVIORS::PERCONA))) {
