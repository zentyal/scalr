			...
			try 
            {
            	...
            	{ 
            		...
            		try
					{
						...
						$authData->auth = new stdClass();
						$authData->auth->tenantName = $this->authProjectName;
						$authData->auth->passwordCredentials = new stdClass();
						$authData->auth->passwordCredentials->username = $this->authUser;
						$authData->auth->passwordCredentials->password = $this->authKey;
						
						$response = $this->sendRequest("{$this->apiVersion}/tokens", "POST", $authData, $this->apiAuthURL);
						$resposne = json_decode($response['body']);
						
						$this->xAuthToken	= $resposne->access->token->id;
						
						foreach ($resposne->access->serviceCatalog as $service) {
							if ($service->type == 'compute') {
								$r = parse_url($service->endpoints[0]->publicURL);
