<?php
	...
	class Scalr_Service_Cloud_Openstack_v2_0_Client extends Scalr_Service_Cloud_Openstack_Connection
	{
        ...
        * @return mixed $response
        */
        public function serverCreate($name, $imageId, $flavorId, $metaData, $personality = array())
        {
             $args['server'] = array(
             	'name'		=> $name,
             	'imageRef'	=> (int)$imageId,
             	'flavorRef'	=> (int)$flavorId,
             	'key_name'  => 'trystack'
             );
             
             if ($metaData)
             	$args['server']['user_data'] = $metaData;
