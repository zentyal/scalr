	...
	public function call ($action, $options, $path = '/')
	{
		...
		if ($httpMethod === 'PUT') {
			if (array_key_exists('putData', $extraOptions)) {
				...
				if (!isset($options['Content-Length'])) {
					$options['Content-Length'] = filesize($extraOptions['putData']);
				}
				if (!isset($options['Content-MD5']) && !empty($options['Content-Length'])) {
					$options['Content-MD5'] = Aws::getMd5Base64DigestFile($extraOptions['putData']);
				}
			}
		}
		...
		$canonicalizedResource = (isset($extraOptions['subdomain']) ? '/' . $extraOptions['subdomain'] : '')
