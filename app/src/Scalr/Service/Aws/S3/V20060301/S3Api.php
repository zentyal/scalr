...
class S3Api
{
	...
	public function listObjects($bucketName, $delimiter = null, $marker = null, $maxKeys = null, $prefix = null)
	{
		...
		$aQueryString = array();
		if ($delimiter !== null) {
			$aQueryString[] = 'delimiter=' . urlencode($delimiter);
		}
		if ($marker !== null) {
			$aQueryString[] = 'marker=' . urlencode($marker);
		}
		if ($maxKeys !== null) {
			$aQueryString[] = 'max-keys=' . urlencode($maxKeys);
		}
		if ($prefix !== null) {
			$aQueryString[] = 'prefix=' . urlencode($prefix);
		}
		...
		if ($response->getError() === false) {
			...
			if (!empty($sxml->Contents)) {
				foreach ($sxml->Contents as $v) {
					...
					$object = $this->s3->object->get($objectName);
	...
	public function deleteObject($bucketName, $objectName, $versionId = null, $xAmfMfa = null)
	{
		...
		$response = $this->client->call('DELETE', $options, '/' . $objectName . ($versionId !== null ? '?versionId=' . (string)$versionId : ''));
		if ($response->getError() !== false) {
