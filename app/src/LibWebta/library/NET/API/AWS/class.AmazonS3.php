<?
	...
	class AmazonS3 
    {
		...
		public function DeleteBucket($bucket_name)
		{
			...
			$data_to_sign = array("DELETE", "", "", $timestamp, "/{$bucket_name}/");
			
			$signature = $this->GetRESTSignature($data_to_sign);
			
			$HttpRequest->setUrl("https://{$bucket_name}.s3.amazonaws.com/");
		    $HttpRequest->setMethod(constant("HTTP_METH_DELETE"));
            ...
            try 
            {
                ...
                else
                {
                	if ($HttpRequest->getResponseBody())
                	{
                		...
                		throw new Exception(_("Cannot delete S3 bucket at this time. Please try again later."));
                }
            }
            catch (Exception $e)
            {            	
