		...
		try {
			...
			$taskList = $db->GetAll("SELECT *
				...
				WHERE
					...
					(
						...
						`last_start_time` IS NOT NULL AND `start_time` IS NULL AND (CONVERT_TZ(last_start_time + INTERVAL (restart_every * 60) SECOND, 'SYSTEM', `timezone`) < CONVERT_TZ(NOW(),'SYSTEM',`timezone`)) OR
						`last_start_time` IS NOT NULL AND `start_time` IS NOT NULL AND (CONVERT_TZ(last_start_time + INTERVAL (restart_every * 0.9 * 60) SECOND, 'SYSTEM', `timezone`) < CONVERT_TZ(NOW(),'SYSTEM',`timezone`))
					)
			...
			foreach ($taskList as $task) {
				...
				if ($task['last_start_time'] && $task['start_time']) {
					...
					$num = ($currentTime->getTimestamp() - $startTime->getTimestamp()) / ($task['restart_every'] * 60);
